...
class BiESN2DClassifier(modules.Classifier):
    def __init__(
        ...
        # BiReservoir2D は縦横 x 順逆で seed を 4 つ消費する
        seeds = modules.per_layer_seeds(seed, [4] * len(layers))
    ...
    def features(self, images):
        ...
        x = self.bi_reservoir2ds(x)  # (B, N_h, N_w, units)

        return x.mean(dim=(1, 2))  # (B, units)


def build(input_shape, num_classes, **kwargs):
    """networks.build_classifier("bi_esn2d", ...) のエントリポイント。

    Args:
        input_shape: 入力画像の形状 (C, H, W)。
        num_classes: クラス数。
    """
    return BiESN2DClassifier(input_shape, num_classes, **kwargs)
